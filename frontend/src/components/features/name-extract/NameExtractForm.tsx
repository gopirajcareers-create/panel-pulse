...
export function NameExtractForm() {
  ...
  const handleDownloadAndEvaluate = async () => {
    // Download any available
    if (results.jd) downloadCSV(results.jd);
    if (results.l1) downloadCSV(results.l1);
    if (results.l2) downloadCSV(results.l2);

    await handleEvaluateDirectly();
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-bg-card rounded-xl border border-white/[0.06] p-6 space-y-6">
        ...
        <div className="flex flex-col gap-3 border-l-2 border-primary/30 pl-3 py-1">
          ...
          <div className="flex items-center gap-2 text-xs text-text-muted italic">
            ...
            <span className="font-bold text-primary border-b border-primary/30 pb-0.5">After extraction is completed for all three files, the Evaluate button will mysteriously appear below!</span>
          </div>
        </div>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 sm:flex sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {evaluationScore !== null ? (
              ...
              <>
                <button
                  onClick={handleDownloadAndEvaluate}
                  disabled={evaluationLoading}
                  className="px-5 py-2 border border-white/10 bg-white/5 hover:bg-white/10 rounded-lg font-medium text-sm text-text-primary transition-colors flex items-center gap-2 disabled:opacity-50"
                >
